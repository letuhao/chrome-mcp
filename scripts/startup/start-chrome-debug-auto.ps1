# PowerShell script to start Chrome with remote debugging (auto-confirm version)

$chromePath = "C:\Program Files\Google\Chrome\Application\chrome.exe"
$userDataDir = "$env:LOCALAPPDATA\Google\Chrome\User Data"

Write-Host "Chrome Remote Debugging Setup (Auto)" -ForegroundColor Cyan
...
if ($chromeRunning) {
    Write-Host "Closing Chrome to restart with remote debugging..." -ForegroundColor Yellow
...
try {
    ...
    Write-Host "Chrome may still be starting. Waiting additional 3 seconds..." -ForegroundColor Yellow
    Start-Sleep -Seconds 3
}

Write-Host ""
Write-Host "Done! Chrome should now be ready." -ForegroundColor Green
